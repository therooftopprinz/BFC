...
{
    EXPECT_TRUE ((std::is_constructible<const_buffer_view,ConstBuffer>::value));
}
TEST(buffer_view, ShouldNotConstructFromBuffer_ConstToNonConst)
{
    EXPECT_FALSE((std::is_constructible<buffer_view,ConstBuffer>::value));
}
...
{
    EXPECT_FALSE((std::is_assignable<buffer_view,ConstBuffer>::value));
}
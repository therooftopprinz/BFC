...
struct FixedFunctionObjectTypeHelper;
template <std::size_t N, typename ReturnType, typename... ArgsType>
struct FixedFunctionObjectTypeHelper<N, ReturnType(ArgsType...)>
{
    using type = FixedFunctionObject<N, ReturnType, ArgsType...>;
};

template <typename FnType>
using UltraLightFn = typename FixedFunctionObjectTypeHelper<8, FnType>::type;
template <typename FnType>
using LightFn = typename FixedFunctionObjectTypeHelper<24, FnType>::type;
template <typename FnType>
using HeavyFn = typename FixedFunctionObjectTypeHelper<32, FnType>::type;
